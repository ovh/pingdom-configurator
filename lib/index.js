...
module.exports = class PingdomConfigurator {
  ...
  async runChecksConfiguration() {
    ...
    await Bluebird.map(checksInConfig, async (checkInConfig) => {
      ...
      // fix for requestheaders format
      if (checkInConfig.requestheaders) {
        Object.entries(checkInConfig.requestheaders).forEach(([key, value], i) => {
          checkInConfig[`requestheaders${i}`] = [`${key}:${value}`];
        });
        delete checkInConfig.requestheaders;
      }
