...
function validateConfig(config) {
  ...
  // validate filterTags if checksCommonConfig have tags
  if (config.filterTags && config.checksCommonConfig && config.checksCommonConfig.tags) {
    const checksCommonConfigTags = config.checksCommonConfig.tags.split(',');
    if (_.difference(config.filterTags, checksCommonConfigTags).length) {
      throw new Error('You must have all the "filterTags" in the tags in checksCommonConfig!');
    }
  }
  // validate filterTags if tmsChecksCommonConfig have tags
  if (config.filterTags && config.tmsChecksCommonConfig && config.tmsChecksCommonConfig.tags) {
    if (_.difference(config.filterTags, config.tmsChecksCommonConfig.tags).length) {
      throw new Error('You must have all the "filterTags" in the tags in tmsChecksCommonConfig!');
    }
  }
}
 ...
 */
function validateChecksConfig(config, checks) {
  ...
  const typesAttributesSetModel = {
    http: Joi.object().keys({
      auth: Joi.string().regex(/^\w+:\w+$/),                  // (string) Username and password colon separated for target HTTP authentication
  ...
  for (const check of checks) {
    ...
    // validate filterRegex
    if (config.filterRegex && !(new RegExp(config.filterRegex)).test(check.name)) {
      throw new Error(`The check name "${check.name}" doesn't match the "filterRegex"!`);
    }
    // validate filterTags if check have tags
    if (config.filterTags && check.tags) {
      const checkTags = check.tags.split(',');
      if (_.difference(config.filterTags, checkTags).length) {
        throw new Error(`You must have all the "filterTags" in your tags of your check "${check.name}"!`);
      }
    }
  }
}
 ...
 */
function validateTmsChecksConfig(config, tmsChecks) {
  const tmsCheckModel = Joi.object().keys({
    ...
    integration_ids: Joi.array().items(Joi.number().integer()),      // (array of integer) Integration identifiers
  ...
  for (const tmsCheck of tmsChecks) {
    ...
    // validate filterRegex
    if (config.filterRegex && !(new RegExp(config.filterRegex)).test(tmsCheck.name)) {
      throw new Error(`The tmsCheck name "${tmsCheck.name}" doesn't match the "filterRegex"!`);
    }
    // validate filterTags if tmsCheck have tags
    if (config.filterTags && tmsCheck.tags) {
      if (_.difference(config.filterTags, tmsCheck.tags).length) {
        throw new Error(`You must have all the "filterTags" in your tags of your tmsCheck "${tmsCheck.name}"!`);
      }
    }
  }
}
